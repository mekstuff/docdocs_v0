...
export function getRequiredPackageInfo(packageRoot) {
    if (!packageRoot) {
        packageRoot = "./";
    }
    ...
    try {
        ...
        assert(typeof name === "string", `package.json in root "${packageRoot}" does not contain a valid "name" field.`);
        assert(typeof version === "string", `package.json in root "${packageRoot}" does not contain a valid "name" field.`);
        return {
            name: name,
            version: version,
            author: author,
