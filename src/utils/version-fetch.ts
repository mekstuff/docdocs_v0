 ...
 */
export function GetPackageVitePressVersion(
  PackageRoot: string,
  FetchLatest?: boolean
): string {
  assert(
    typeof PackageRoot === "string",
    `PackageRoot expected to be string, got ${PackageRoot}`
  );
  ...
  if (fs.existsSync(PackageJSONPath)) {
    try {
      ...
      console.warn(
        chalk.yellow(
          `Something went wrong when getting vitepress version from ${PackageJSONPath}. => ${e}`
        )
      );
    }
  }
 ...
 */
export function GetCurrentDocDocsVersion(FetchLatest?: boolean): string {
