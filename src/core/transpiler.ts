...
import { ISerializedClassNode } from "@mekstuff/docdocs-parser-typescript";

import { h1, h2, br, lb, h3 } from "@mekstuff/cmkdn";
import { ISerializedSymbol } from "@mekstuff/docdocs-parser-typescript/dist/utils/main-serializer.js";
...
export async function WriteApiFiles(
  ...
  ApiClassEntries: Map<string, ISerializedClassNode[]>
) {
  ...
  await RemoveAPIDirectory(PackageDirectory, vc);
  ApiClassEntries.forEach(async (v, file) => {
    const CurrentApiFileLogger = Console.progress.bar();
    v.forEach(async (x, i) => {
      CurrentApiFileLogger((v.length - i) * 100, x.symbol.name);
      await AddAPIClass(PackageDirectory, file, x);
    });
    CurrentApiFileLogger(100, "Done.");
  });
}
...
export async function ReadViteConfig(
  ...
  try {
    ...
    Console.error("Something went wrong when reading config file " + e);
  }
}
...
export async function WriteViteConfig(
  ...
  `;
    ...
    Console.error("Something went wrong when writing config file " + e);
  }
}
...
async function RemoveAPIRoute(
  ...
  if (ViteConfig === undefined) {
    return Console.error("Something went wrong when reading viteconfig.");
  }
...
export async function AddAPIClass(
  PackageDirectory: string,
  Path: string,
  Class: ISerializedClassNode
) {
  const ClassMarkdown = await FromClassToMarkdown(Class);
  ...
  // console.log(h3("Hello".repeat(4)));
}

/**
 * Creates markdown text based on class
 */
async function FromClassToMarkdown(
  Class: ISerializedClassNode
): Promise<string> {
  let Markdown = h1(Class.symbol.name);
  const Properties: ISerializedSymbol[] = [];
  const Methods: ISerializedSymbol[] = [];
  Class.members.forEach((x) => {
    if (x.isProperty) {
      Properties.push(x);
    }
    if (x.isMethod) {
  ...
  Markdown += br();
  Markdown += ParseDocumenationString(Class.symbol.documentation);
  if (Properties.length > 0) {
    Markdown += lb(h2("Properties"));
    Markdown += br();
    await Properties.forEach(async (x) => {
      Markdown += await FromPropertyMemberToMarkdown(Class.symbol.name, x);
    });
  }
...
/**
 * Creates markdown text for a property members
 */
async function FromPropertyMemberToMarkdown(
  ClassName: string,
  PropertyMember: ISerializedSymbol
): Promise<string> {
  const Markdown = `${h3(PropertyMember.name)}
\`${ClassName}.${PropertyMember.name}: ${PropertyMember.type}\`

${ParseDocumenationString(PropertyMember.documentation)}
...
function ParseDocumenationString(Documenation: string): string {
  return Documenation;
}
